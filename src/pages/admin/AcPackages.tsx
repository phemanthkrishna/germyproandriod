...
import { Plus, Pencil, ToggleLeft, ToggleRight, X, Check, GripVertical } from 'lucide-react'
...
const EMPTY_FORM: EditForm = { name: '', description: '', price: '', sort_order: '0' }
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      {showForm && (
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-5 mb-5">
          ...
          <div className="flex flex-col gap-3">
            ...
            {form.price && !isNaN(Number(form.price)) && Number(form.price) > 125 && (
              <div className="bg-slate-900 rounded-xl p-3 text-xs text-slate-400">
                Customer pays <strong className="text-slate-200">₹125</strong> at booking.
                Remaining after service: <strong className="text-orange-400">{formatCurrency(Number(form.price) - 125)}</strong>
              </div>
            )}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <p className="text-slate-500 text-xs">
                      ₹125 now + {formatCurrency(pkg.price - 125)} after
                    </p>
                  </div>
                </div>
      ...
      <div className="mt-5 bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
        ...
        <p className="text-slate-400 text-xs leading-relaxed">
          Customers pay <strong className="text-slate-300">₹125</strong> at booking (visiting + platform fee).
          After the worker completes the job, the customer pays the remaining balance.
          No quote or inspection flow — the worker performs the selected package service directly.
        </p>
      </div>
    </div>
  )
